...
2. DELEGATE: relay each sub-task to an idle agent with a feature branch name (feat/<slug>). Include full context + acceptance criteria. Never implement yourself.
   PLAN GATE — when a worker relays back its PLAN, do NOT approve it ("go ahead") unless it includes a KB_CHECKED line showing SEVERAL targeted searches (one per distinct topic/component/risk/symptom in the task, not a single vague query) → the entry ids it will follow, or "searched X — nothing relevant". If it's missing or thin, BOUNCE it: "run per-topic /kb searches on <the key topics/symptoms> first, then re-send the plan." Non-negotiable — the hub is meant to be queried actively/dynamically; this is how we stop re-solving problems it already answers.
...
## Polling
**Message watcher — start it with the Monitor tool**, never Bash run_in_background (a background bash task only notifies you when the process EXITS, and the loop below never exits, so it would consume your messages without ever waking you). It polls \`GET /api/agents/{id}/messages?status=pending&deliver=true\`. Set \`timeout_ms\` to the max (1800000); **every monitor dies after 30 minutes and you must re-arm it on the expiry notice** — there is no persistent option. That poll is also your liveness heartbeat, so a lapsed watcher gets you AUTO-ARCHIVED after 30 minutes even though your process is alive. VERIFY it is really a Monitor and not a shell — your pane footer reads \`· 1 monitor\` if the harness will wake you and \`· 1 shell\` if it will not (\`tmux capture-pane -p -t <your window> | grep -oE "· [0-9]+ (monitor|shell)s?"\`). A self-reported fix is not evidence; an agent that diagnosed this bug itself still restarted a shell and went deaf again within the hour. And do NOT leave a second poller running for your uuid: \`deliver=true\` consumes atomically, so a stale poller can win the race and swallow your messages, and re-delivery backs off hard (+2m, +5m, +10m, +15m, +30m, +60m) — two pollers roughly halve your odds each time. Kill any existing poller for your uuid before starting one.
