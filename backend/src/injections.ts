...
8. KNOWLEDGE HUB: search it before non-trivial work / when unsure — /kb <question> (or GET $AGENT_URL/api/kb/search?q=). Use approved hits (cite id); pending=unverified. On a miss, contribute: POST $AGENT_URL/api/kb/propose (→ approval queue). People facts → POST $AGENT_URL/api/kb/profiles.
...
16. KNOWLEDGE HUB (shared hive mind): Before non-trivial work or when unsure how our systems/conventions work, SEARCH FIRST — /kb <question> or GET $AGENT_URL/api/kb/search?q=. Use approved results (cite the id); treat pending as unverified; verify volatile facts (paths/flags) live. On a miss, research then CONTRIBUTE: POST $AGENT_URL/api/kb/propose {"kind":"new","title","body","tags":[],"systems":[],"agent":"<you>","rationale"} (→ the user's approval queue). Log facts about people via POST $AGENT_URL/api/kb/profiles (auto-applied, no approval). Every agent both uses AND feeds the hub.
Silence = the user cannot see what you are doing.
---`;
...
12b. KNOWLEDGE HUB (shared hive mind): Before non-trivial work or when unsure how our systems/conventions work, SEARCH FIRST — /kb <question> or GET $AGENT_URL/api/kb/search?q=. Use approved results (cite the id); treat pending as unverified; verify volatile facts live. On a miss, research then CONTRIBUTE it back: POST $AGENT_URL/api/kb/propose {"kind":"new","title","body","tags":[],"systems":[],"agent":"<you>","rationale"} (→ the user's approval queue). Record fixes/gotchas the same way. Log facts about people via POST $AGENT_URL/api/kb/profiles (auto-applied). You both USE and FEED the hub — this is how the fleet stays consistent and stops repeating mistakes.
