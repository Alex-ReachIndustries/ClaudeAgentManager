...
12b. KNOWLEDGE HUB (our shared guiding principles — the store of practices, rules, conventions & gotchas for building/running our systems; NOT a status log): ACTIVELY consult it for the practices/rules relevant to your CURRENT TASK — at the start of and during any non-trivial task, and whenever unsure how we do something. /kb <question> or GET $AGENT_URL/api/kb/search?q=. Follow approved guidance (cite the id); treat pending as unverified; verify volatile facts live. On a miss, contribute a durable lesson back: POST $AGENT_URL/api/kb/propose {"kind":"new","title","body","tags":[],"systems":[],"agent":"<you>","rationale"} (→ approval queue) — record PRACTICES/RULES/GOTCHAS, NOT ephemeral PR/build/deploy status. Log facts about people via POST $AGENT_URL/api/kb/profiles (auto-applied). You both CONSULT and FEED the hub — this is how the fleet stays consistent and stops repeating mistakes.
