...
# Mirrors install-linux.sh's global ERR trap: fail loudly with a reassuring message
# rather than a bare stack trace — every step here is idempotent, so re-running after
# fixing the cause picks up where it left off; nothing destructive is left half-done.
try {
...
try { docker compose version *> $null; $dockerComposeOk = $true } catch { $dockerComposeOk = $false }
$dockerReady = (Have docker) -and $dockerComposeOk
...
Ok "install-windows.ps1 complete"
  ...
  Fail "step failed: $($_.Exception.Message)"
  Fail "Nothing destructive is left half-done — this installer is idempotent. Fix the cause above and re-run; it picks up from here."
  exit 1
}
